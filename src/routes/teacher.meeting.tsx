                    ...
                    <div className="p-6 md:p-10">
                      {selectedPastMeeting ? (
                        ...
                        `}</style>
                          ...
                          <div id="meeting-pdf-content" className="register-container">
                            {isEditing ? (
                            ...
                            <div className="pt-8 border-t border-slate-100 space-y-6">
                              ...
                              {showCustomForm && (
                                <div className="space-y-6 font-sans">
                                  ...
                                  <div className="overflow-x-auto bg-white rounded-xl border-2 border-slate-300 shadow-sm">
                                    <table className="w-full border-collapse min-w-[800px]">
                                      <thead>
                                        <tr className="bg-slate-100 border-b-2 border-slate-300">
                                          <th className="p-3 text-center font-black text-slate-800 border-r-2 border-slate-300 w-24">अ.क्र.</th>
                                          <th className="p-3 text-left font-black text-slate-800 border-r-2 border-slate-300 w-1/4">विषय</th>
                                          <th className="p-3 text-left font-black text-slate-800 border-r-2 border-slate-300 w-[40%]">ठराव तपशील</th>
                                          <th className="p-3 text-left font-black text-slate-800 border-r-2 border-slate-300 w-1/5">सूचक व अनुमोदक</th>
                                          <th className="p-3 text-center font-black text-slate-800 w-16">कृती</th>
                                        </tr>
                                      </thead>
                                      <tbody>
                                        {formResolutions.map((res: any, index: number) => (
                                          <tr key={index} className="border-b-2 border-slate-200 hover:bg-slate-50 transition-colors">
                                            <td className="p-3 align-top border-r-2 border-slate-300 bg-slate-50/50">
                                              <div className="space-y-4">
                                                <div>
                                                  <label className="text-[10px] text-slate-500 font-bold mb-1 block uppercase tracking-wider text-center">विषय क्र.</label>
                                                  <input type="number" value={res.subjectNo} onChange={(e) => handleUpdateFormResolutionField(index, "subjectNo", Number(e.target.value))} className="w-full px-2 py-1.5 border-2 border-slate-300 rounded focus:ring-2 focus:ring-blue-600 outline-none font-bold text-center" />
                                                </div>
                                                <div>
                                                  <label className="text-[10px] text-slate-500 font-bold mb-1 block uppercase tracking-wider text-center">ठराव क्र.</label>
                                                  <input type="number" value={res.resolutionNo} onChange={(e) => handleUpdateFormResolutionField(index, "resolutionNo", Number(e.target.value))} className="w-full px-2 py-1.5 border-2 border-slate-300 rounded focus:ring-2 focus:ring-blue-600 outline-none font-bold text-center" />
                                                </div>
                                              </div>
                                            </td>
                                            <td className="p-3 align-top border-r-2 border-slate-300">
                                              <textarea
                                                value={res.subject}
                                                onChange={(e) => handleUpdateFormResolutionField(index, "subject", e.target.value)}
                                                placeholder="उदा. सभेचे इतिवृत्त..."
                                                className="w-full h-full min-h-[160px] p-3 border-2 border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-600 outline-none resize-y font-bold text-slate-800 leading-relaxed bg-white"
                                              />
                                            </td>
                                            <td className="p-3 align-top border-r-2 border-slate-300">
                                              <textarea
                                                value={res.resolution || ""}
                                                onChange={(e) => handleUpdateFormResolutionField(index, "resolution", e.target.value)}
                                            ...
                                            <td className="p-3 align-top border-r-2 border-slate-300 bg-slate-50/50">
                                              <div className="space-y-4">
                                                <div>
                                                  <label className="text-xs text-slate-500 font-bold mb-1.5 block">सूचक</label>
                                                  <select value={res.proposer} onChange={(e) => handleUpdateFormResolutionField(index, "proposer", e.target.value)} className="w-full p-2 border-2 border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-600 outline-none text-sm font-bold text-slate-800 cursor-pointer bg-white">
                                                    <option value="">-- निवडा --</option>
                                                    {formMembers.map((m: any, mIdx: number) => m.name && (<option key={mIdx} value={m.name}>{m.name}</option>))}
                                                  </select>
                                                </div>
                                                <div>
                                                  <label className="text-xs text-slate-500 font-bold mb-1.5 block">अनुमोदक</label>
                                                  <select value={res.seconder} onChange={(e) => handleUpdateFormResolutionField(index, "seconder", e.target.value)} className="w-full p-2 border-2 border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-600 outline-none text-sm font-bold text-slate-800 cursor-pointer bg-white">
                                                    <option value="">-- निवडा --</option>
                                                    {formMembers.map((m: any, mIdx: number) => m.name && (<option key={mIdx} value={m.name}>{m.name}</option>))}
                                                  </select>
                                                </div>
                                            ...
                                            <td className="p-3 align-middle text-center">
                                              <button
                                                type="button"
                                                onClick={() => handleRemoveFormResolutionRow(index)}
                                                className="p-2.5 text-red-500 hover:bg-red-50 hover:text-red-700 rounded-xl transition-colors border-2 border-transparent hover:border-red-200 shadow-sm hover:shadow"
                                    ...
                                    {formResolutions.length === 0 && (
                                      <div className="text-center p-8 text-slate-500 font-bold bg-white">
                                        कोणतेही विषय जोडलेले नाहीत. कृपया 'विषय व ठराव जोडा' वर क्लिक करा.
                                      </div>
                                    )}
                                  </div>
