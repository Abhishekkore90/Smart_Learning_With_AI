...
function TeacherMDMPage() {
  ...
  useEffect(() => {
    ...
    if (p) {
      if (p.schoolName) setReportSchoolName(p.schoolName);
      if (p.cookName || p.swayampakiName) setReportPrincipalName(p.cookName || p.swayampakiName);
      else if (p.principalName) setReportPrincipalName(p.principalName);

      if (p.helperName || p.madatnisName) setReportTeacherName(p.helperName || p.madatnisName);
      else if (p.teacherName) setReportTeacherName(p.teacherName);
  ...
  const handleAnnualReportDownload = async () => {
    ...
    try {
      ...
      if (!isGrainReport) {
        ...
        tableHtmlStr = grainTablesHtmlStr;
      }
  ...
  const getLastDayOfMonth = (yearStr: string, monthNameStr: string): string => {
    const monthNames = [
      "January", "February", "March", "April", "May", "June",
      "July", "August", "September", "October", "November", "December"
    ];
    const mIndex = monthNames.findIndex(
      (m) => m.toLowerCase() === (monthNameStr || "").trim().toLowerCase()
    );
  ...
  const [stockAsOnDate, setStockAsOnDate] = useState(() => getLastDayOfMonth("2026", "August"));
  ...
  const isSyncingTable = React.useRef(false);
  const [tableScrollWidth, setTableScrollWidth] = useState(2550);

  const handleTopScroll = () => {
    if (!isSyncingTop.current && tableScrollRef.current && topScrollRef.current) {
  ...
  const handleTableScroll = () => {
    if (!isSyncingTable.current && tableScrollRef.current && topScrollRef.current) {
  ...
  useEffect(() => {
    if (tableScrollRef.current) {
      setTableScrollWidth(tableScrollRef.current.scrollWidth || 2550);
    }
  }, [registerRecords, registerDate, registerClass, activeTab]);
  ...
  const [openingStockDateMap, setOpeningStockDateMap] = useState<
    Record<
      string,
      {
        ...
        borrowedIn?: Record<string, string>;
        borrowedOut?: Record<string, string>;
        loksahabhag?: Record<string, string>;
      }
    >
  >({});

  // Derived Earliest Initial Opening Stock Date (आरंभीची शिल्लक नोंदवलेली पहिली तारीख)
  const initialStockDate = useMemo(() => {
    const dates = Object.keys(openingStockDateMap || {}).filter(
  ...
  }, [openingStockDateMap, openingStockDate]);
  ...
  // Auto-sync opening stock inputs when openingStockDate or initialStockDate changes
  useEffect(() => {
    if (!openingStockDate) return;
    if (initialStockDate && openingStockDate < initialStockDate) {
      setOpeningStockValues({
        Rice: "0", Mugdal: "0", Turdal: "0", Masurdal: "0", Matki: "0", Moong: "0",
        Cowpea: "0", Gram: "0", Pease: "0", "Soyabean Wadi": "0", Cumin: "0", Mustard: "0",
        Turmeric: "0", Chili: "0", "Onion Garlic Masala": "0", "Garam Masala": "0", Oil: "0", Salt: "0"
      });
      setOpeningStockBorrowedIn({});
      setOpeningStockBorrowedOut({});
      setOpeningStockLoksahabhag({});
    } else if (openingStockDateMap[openingStockDate]?.values) {
      const record = openingStockDateMap[openingStockDate];
      if (record.values) setOpeningStockValues(record.values);
      if (record.borrowedIn) setOpeningStockBorrowedIn(record.borrowedIn);
      if (record.borrowedOut) setOpeningStockBorrowedOut(record.borrowedOut);
      if (record.loksahabhag) setOpeningStockLoksahabhag(record.loksahabhag);
    } else {
      setOpeningStockValues({});
      setOpeningStockBorrowedIn({});
      setOpeningStockBorrowedOut({});
  ...
  }, [openingStockDate, openingStockDateMap, initialStockDate]);
  ...
  const handleSaveOpeningStock = async () => {
    ...
    try {
      const udise = getUdise();
      const isInitialDate = !initialStockDate || openingStockDate === initialStockDate;
      const targetInitDate = initialStockDate || openingStockDate;
      const updatedDateMap = {
        ...
        ...(targetInitDate ? {
          [targetInitDate]: {
            ...(openingStockDateMap[targetInitDate] || {}),
            values: openingStockValues,
          }
        } : {})
      };
  ...
  const handleExportOpeningStockExcel = async () => {
    try {
      ...
      const rows = REPORT_ITEMS.slice(0, 18).map((item, idx) => {
        ...
        const isInitialDate = !initialStockDate || openingStockDate === initialStockDate;
        const hasSavedCustomRecord = isInitialDate && !!(openingStockDateMap[openingStockDate]?.values && Object.keys(openingStockDateMap[openingStockDate].values).length > 0);
        const hasInputVal = openingStockValues[item.key] !== undefined && openingStockValues[item.key] !== "" && !isNaN(parseFloat(openingStockValues[item.key]));
        const openVal = isPriorToInitial ? 0 : (hasSavedCustomRecord && hasInputVal ? roundStock(parseFloat(openingStockValues[item.key])) : calculatedOpening);
        ...
        const usedQty = roundStock(getUsedForMonth(targetMo, targetYr, "1 To 5", item.key));
        const availStock = roundStock(openVal + borInVal + lokVal);
        const isInvalid = borOutVal > availStock && borOutVal > 0;
        const totalStock = isInvalid ? 0 : Math.max(0, roundStock(availStock - borOutVal - usedQty));
        ...
        return {
          ...
          "उसना दिला (-)": borOutVal,
          "लोकसहभाग (+)": lokVal,
          "वापरलेला साठा (-)": usedQty,
          "महिनाअखेर शिल्लक साठा (=)": isInvalid ? "Invalid Entry" : totalStock,
        };
      });
  ...
  const handleOpeningStockPrint = () => {
    ...
    printWindow.document.write(`
          ...
          <table>
            <thead>
              <tr>
                ...
                <th class="text-center">वापरलेला साठा (-)</th>
                <th class="text-center">महिनाअखेर शिल्लक साठा (=)</th>
              </tr>
            </thead>
            <tbody>
      ...
      const isPriorToInitial = !!(initialStockDate && openingStockDate && openingStockDate < initialStockDate);
      const calculatedOpening = roundStock(getOpeningStock(targetMo, targetYr, "1 To 5", item.key));
      const hasSavedCustomRecord = !!(openingStockDateMap[openingStockDate]?.values && Object.keys(openingStockDateMap[openingStockDate].values).length > 0);
      const hasInputVal = openingStockValues[item.key] !== undefined && openingStockValues[item.key] !== "" && !isNaN(parseFloat(openingStockValues[item.key]));
      const openVal = isPriorToInitial ? 0 : (hasSavedCustomRecord && hasInputVal ? roundStock(parseFloat(openingStockValues[item.key])) : calculatedOpening);
      const borInVal = isPriorToInitial ? 0 : Math.max(0, parseFloat(openingStockBorrowedIn[item.key] || "0") || 0);
      const borOutVal = isPriorToInitial ? 0 : Math.max(0, parseFloat(openingStockBorrowedOut[item.key] || "0") || 0);
      const lokVal = isPriorToInitial ? 0 : Math.max(0, parseFloat(openingStockLoksahabhag[item.key] || "0") || 0);
      const usedQty = roundStock(getUsedForMonth(targetMo, targetYr, "1 To 5", item.key));
      ...
      return `
                  <tr>
                    ...
                    <td class="text-center font-bold text-orange-900">${usedQty}</td>
  ...
  useEffect(() => {
    if (
      ...
      if (incomingDates && incomingDates[recordKey]) {
        setIncomingDate(incomingDates[recordKey]);
      } else {
  ...
  }, [incomingYear, incomingMonth, incomingClass, incomingRecords, incomingDates]);
  ...
  const getUsedForMonth = (
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (initialStockDate && dateISO < initialStockDate) continue;
  ...
  const getInitialOpeningStockValue = (name: string): number => {
    ...
    if (initialStockDate && openingStockDateMap[initialStockDate]?.values) {
      const initVals = openingStockDateMap[initialStockDate].values;
      const vStr = initVals[name] || initVals[itemKey];
      if (vStr !== undefined && vStr !== "" && !isNaN(parseFloat(vStr))) {
        return parseFloat(vStr);
      }
    }
    if (openingStockDateMap) {
      const sortedKeys = Object.keys(openingStockDateMap).sort();
      for (const dKey of sortedKeys) {
        const vals = openingStockDateMap[dKey]?.values;
        if (vals) {
          const vStr = vals[name] || vals[itemKey];
          if (vStr !== undefined && vStr !== "" && !isNaN(parseFloat(vStr))) {
            return parseFloat(vStr);
          }
        }
  ...
  const getOpeningStock = (
    ...
    if (initialStockDate) {
      const parts = initialStockDate.split("-");
      if (parts.length === 3) {
        const initYear = parts[0];
        const initMonthNum = parseInt(parts[1], 10);
        ...
        const evalMonthNum = monthMap[monthName] || monthNamesEng.findIndex(m => m && m.toLowerCase() === monthName.toLowerCase());
  ...
  useEffect(() => {
    ...
    REPORT_ITEMS.forEach((item) => {
      ...
      // Auto Carry Forward: On initial stock date, use stored custom values if present. On all other dates, carry forward calculated opening stock for that month.
      const isInitialDate = !initialStockDate || targetDate === initialStockDate;
      if (isInitialDate && storedDateData?.values?.[item.key] !== undefined && storedDateData.values[item.key] !== "") {
        newValues[item.key] = storedDateData.values[item.key];
      } else {
        const carriedVal = getOpeningStock(targetMonth, targetYear, "1 To 5", item.key);
        newValues[item.key] = carriedVal > 0 ? carriedVal.toString() : (storedDateData?.values?.[item.key] || "0");
      }
    });
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          if (firestoreData.incomingDates) {
            setIncomingDates(firestoreData.incomingDates);
          }
  ...
  const handleSaveIncoming = async () => {
    ...
    try {
      ...
      const updatedDates = {
        ...incomingDates,
        [recordKey]: incomingDate,
      };
      setIncomingDates(updatedDates);
  ...
  const handleViewStockData = () => {
    ...
    const recordKey = `${stockYear}_${stockMonth}_${stockClass}`;
    const computed = computeStockRecordsForKey(recordKey);
    if (computed && computed.length > 0) {
      setStockRecords(computed);
    }
    setShowStockTable(true);
  };

  // Dynamically recalculate stockRecords for Current Stock (सध्याचा साठा) whenever dependencies change
  useEffect(() => {
    if (activeTab === "stock" || showStockTable) {
      const recordKey = `${stockYear}_${stockMonth}_${stockClass}`;
      const computed = computeStockRecordsForKey(recordKey);
      if (computed && computed.length > 0) {
        setStockRecords(computed);
  ...
  const computeStockRecordsForKey = (
    ...
    // Check if currently selected stockAsOnDate is prior to initialStockDate
    const isPriorToInitial = !!(
      initialStockDate &&
      stockAsOnDate &&
      stockAsOnDate < initialStockDate
    );
    ...
    return itemsList.map((itemName) => {
      ...
      // 1. Received this month up to stockAsOnDate (Matrix + incRecords)
      let incTabQty = 0;
      const itemKey = getItemKeyFromName(itemName);
      incRecords.forEach((r) => {
        if (!r.date || !r.item || !r.qty) return;
      ...
      // 3. Used this month up to stockAsOnDate
      let totalUsedKg = 0;
      let cookedDays = 0;
      let benefSum = 0;
      const seenDates = new Set<string>();
      ...
      const lokQty = getLokForMonth(itemName, month, parseInt(year, 10), stockAsOnDate);
      const damaged = getDamagedForMonth(itemName, month, parseInt(year, 10), stockAsOnDate);
                      ...
                      {[
                        ...
                        return (
                          <button
                            ...
                            className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200 border ${isActive
                        ...
                        {[
                          ...
                          return (
                            <button
                              key={sub.id}
                              onClick={() => setActiveTab(sub.id)}
                              className={`flex items-center justify-center gap-1.5 px-2 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all duration-200 border text-center ${isActive
                        ...
                        {[
                          ...
                          return (
                            <button
                              key={sub.id}
                              onClick={() => setActiveTab(sub.id)}
                              className={`flex items-center justify-center gap-2 px-3 py-3 rounded-xl text-sm font-bold transition-all duration-200 border ${isActive
                        ...
                        {[
                          ...
                          return (
                            <button
                              key={sub.id}
                              onClick={() => setActiveTab(sub.id)}
                              className={`flex items-center justify-center gap-1.5 px-2.5 py-3 rounded-xl text-sm font-bold transition-all duration-200 border ${isActive
                      ...
                      <div className="space-y-5 font-sans text-slate-800">
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                          <div className="flex flex-wrap items-end gap-4">
                            ...
                            <div>
                              <label className="text-xs font-semibold text-slate-600 block mb-1.5">इयत्ता गट</label>
                              <select
                                value={calClass}
                                onChange={(e) => setCalClass(e.target.value as "1 To 5" | "6 To 8")}
                                className="h-10 px-4 border border-indigo-400 rounded-lg text-sm font-bold bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 min-w-[170px]"
                              >
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-5">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <label
                              ...
                              className={`p-4 rounded-xl border cursor-pointer transition-all duration-200 flex items-start gap-3.5 ${calMode === "admin"
                            ...
                            <label
                              ...
                              className={`p-4 rounded-xl border cursor-pointer transition-all duration-200 flex items-start gap-3.5 ${calMode === "custom"
                                ? "bg-teal-50/70 border-teal-500 shadow-sm"
                                : "bg-white border-slate-200 hover:border-slate-300"
                                }`}
                            >
                                    ...
                                    <tbody className="divide-y divide-slate-200 text-xs font-sans">
                                      {Array.from({ length: daysInMonth }, (_, i) => {
                                        ...
                                        return (
                                          <tr
                                            key={dayNum}
                                            className={`transition-colors ${entry.isHoliday
                                              ? "bg-rose-50/60"
                    ...
                    {activeTab === "opening-stock" && (
                      <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-2xl border border-emerald-200 shadow-md overflow-hidden">
                          ...
                          {/* Date Status Banner for Opening Stock */}
                          {initialStockDate && openingStockDate && openingStockDate < initialStockDate && (
                            <div className="p-3 bg-amber-50 border border-amber-300 rounded-xl text-xs font-bold text-amber-900 flex items-center gap-2">
                              ...
                              <span>
                                टीप: निवडलेली तारीख ({openingStockDate}) ही आरंभीची शिल्लक नोंदवलेल्या दिनांकाच्या ({initialStockDate}) आधीची असल्यामुळे साठा ० (0) दर्शवत आहे.
                              </span>
                            </div>
                          )}
                            ...
                            <table className="w-full text-left text-xs border-collapse min-w-[800px]">
                              <thead>
                                <tr className="bg-emerald-100/90 border-b border-emerald-300 text-emerald-950 font-bold">
                                  ...
                                  <th className="p-3 text-center border-r border-slate-200 bg-orange-100/80 min-w-[140px]">
                                    वापरलेला साठा (-) <br /><span className="text-[10px] font-normal text-orange-950">(दैनंदिन नोंद)</span>
                                  </th>
                                  <th className="p-3 text-center border-r border-emerald-300 bg-emerald-300/70 font-black text-emerald-950 min-w-[160px]">
                                    महिनाअखेर शिल्लक साठा (=) <br /><span className="text-[10px] font-normal text-emerald-950">(३०/३१ तारीख)</span>
                                  </th>
                                </tr>
                              </thead>
                              <tbody className="divide-y divide-slate-200">
                                {REPORT_ITEMS.slice(0, 18).map((item, idx) => {
                                  ...
                                  const borInVal = isPriorToInitial ? 0 : roundStock(parseFloat(openingStockBorrowedIn[item.key] || "0"));
                                  const borOutVal = isPriorToInitial ? 0 : roundStock(parseFloat(openingStockBorrowedOut[item.key] || "0"));
                                  const calculatedLok = roundStock(getLokForMonth(item.key, targetMo, parseInt(targetYr, 10)));
                                  const lokVal = isPriorToInitial ? 0 : (hasSavedCustomRecord && openingStockLoksahabhag[item.key] !== undefined && openingStockLoksahabhag[item.key] !== "" ? roundStock(parseFloat(openingStockLoksahabhag[item.key])) : calculatedLok);
                                  const usedQty = roundStock(getUsedForMonth(targetMo, targetYr, "1 To 5", item.key));
                                    ...
                                    <tr key={item.key} className={`hover:bg-emerald-50/40 transition-colors ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                                      ...
                                      <td className="p-2 border-r border-slate-200 text-center bg-emerald-50/30">
                                        <input
                                          ...
                                          min="0"
                                          placeholder="0"
                                          value={isPriorToInitial ? "0" : (hasSavedCustomRecord && hasInputVal ? openingStockValues[item.key] : calculatedOpening.toString())}
                                      ...
                                      <td className="p-2 border-r border-slate-200 text-center bg-blue-50/30">
                                        <input
                                          ...
                                          min="0"
                                          placeholder="0"
                                          value={isPriorToInitial ? "0" : (openingStockBorrowedIn[item.key] || "")}
                                          onChange={(e) => handleOpeningBorrowedInChange(item.key, e.target.value)}
                                          disabled={isPriorToInitial}
                                          className="w-full h-9 text-center border border-blue-300 rounded-lg font-bold text-blue-950 focus:ring-2 focus:ring-blue-500 bg-white text-xs px-2 shadow-2xs outline-none disabled:bg-slate-100 disabled:text-slate-400"
                                        />
                                      </td>
                                      ...
                                      <td className="p-2 border-r border-slate-200 text-center bg-rose-50/30">
                                        <input
                                          ...
                                          min="0"
                                          placeholder="0"
                                          value={isPriorToInitial ? "0" : (openingStockBorrowedOut[item.key] || "")}
                                          onChange={(e) => handleOpeningBorrowedOutChange(item.key, e.target.value)}
                                          disabled={isPriorToInitial}
                                          className={`w-full h-9 text-center border rounded-lg font-bold text-xs px-2 shadow-2xs outline-none transition-all disabled:bg-slate-100 disabled:text-slate-400 ${isInvalid
                                      ...
                                      <td className="p-2 border-r border-slate-200 text-center bg-amber-50/30">
                                        <input
                                          ...
                                          min="0"
                                          placeholder="0"
                                          value={isPriorToInitial ? "0" : (openingStockLoksahabhag[item.key] || "")}
                                          onChange={(e) => handleOpeningLoksahabhagChange(item.key, e.target.value)}
                                          disabled={isPriorToInitial}
                                          className="w-full h-9 text-center border border-amber-300 rounded-lg font-bold text-amber-950 focus:ring-2 focus:ring-amber-500 bg-white text-xs px-2 shadow-2xs outline-none disabled:bg-slate-100 disabled:text-slate-400"
                                        />
                                      </td>

                                      {/* 6. वापरलेला साठा (-) */}
                                      <td className="p-2 border-r border-slate-200 text-center bg-orange-50/50">
                                        <div className="w-full h-9 flex items-center justify-center font-bold text-xs text-orange-950 bg-orange-100/40 rounded-lg border border-orange-200 select-none">
                                          {toMarathiNumbers(usedQty.toFixed(3))} {item.unit}
                                        </div>
                                      </td>

                                      {/* 7. महिनाअखेर शिल्लक साठा (Read-only / Autocalculated) */}
                                      <td className="p-2 border-r border-emerald-300 text-center bg-emerald-100/40">
                                        <input
                                          ...
                                          className={`w-full h-9 text-center font-black text-xs px-2 rounded-lg border shadow-inner select-none cursor-not-allowed ${isInvalid
                          ...
                          {(() => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-6 py-3 font-extrabold text-sm rounded-xl shadow-lg transition-all flex items-center gap-2 ${hasInvalid
                          ...
                          <table className="w-full text-xs border-collapse border border-slate-300">
                            <thead>
                              <tr className="bg-emerald-100 border-b border-slate-300 text-emerald-950 font-bold">
                                ...
                                <th className="p-2 border border-slate-300 text-center">वापरलेला साठा (-)</th>
                                <th className="p-2 border border-slate-300 text-center">महिनाअखेर शिल्लक साठा (=)</th>
                              </tr>
                            </thead>
                            <tbody>
                              {REPORT_ITEMS.slice(0, 18).map((item, idx) => {
                                ...
                                return (
                                  <tr key={item.key} className="border-b border-slate-200">
                                    ...
                                    <td className="p-2 border border-slate-300 text-center font-bold text-orange-950">{usedQty}</td>
                        ...
                        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6">
                          ...
                          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full md:w-auto">
                              {/* Class / इयत्ता गट */}
                              <div className="flex items-center gap-2">
                                <label className="text-xs font-black text-slate-700">{lang === "mr" ? "इयत्ता गट:" : "Class Section:"}</label>
                                <select
                                  value={incomingClass}
                                ...
                                >
                                  <option value="1 To 5">{lang === "mr" ? "इयत्ता १ ली ते ५ वी (प्राथमिक)" : "1 To 5 (Primary)"}</option>
                                  <option value="6 To 8">{lang === "mr" ? "इयत्ता ६ वी ते ८ वी (उच्च प्राथमिक)" : "6 To 8 (Upper Primary)"}</option>
                                </select>
                              </div>

                              {/* Stock Arrival Date / साहित्य आवक तारीख */}
                              <div className="flex items-center gap-2">
                                <label className="text-xs font-black text-slate-700">{lang === "mr" ? "साहित्य आवक तारीख:" : "Arrival Date:"}</label>
                                <input
                                  type="date"
                                  value={incomingDate}
                                  onChange={(e) => {
                                    ...
                                    if (val) {
                                      ...
                                      if (parts.length === 3) {
                                        ...
                                        if (moIndex >= 0 && moIndex <= 11) {
                                          ...
                                          setIncomingYear(yr);
                                          setIncomingMonth(mo);
                                          const recordKey = `${yr}_${mo}_${incomingClass}`;
                                          setIncomingDates((prev) => ({ ...prev, [recordKey]: val }));
                                        }
                                      }
                                    }
                                  }}
                          ...
                          {/* Dynamic Date Status Alert Banner */}
                          {(() => {
                            const currentRecKey = `${incomingYear}_${incomingMonth}_${incomingClass}`;
                            const savedArrivalDate = incomingDates[currentRecKey];
                            const isBeforeInitial = !!(initialStockDate && incomingDate && incomingDate < initialStockDate);
                            const isBeforeArrival = savedArrivalDate && incomingDate && incomingDate < savedArrivalDate;
                            const isSameArrival = savedArrivalDate && incomingDate && incomingDate === savedArrivalDate;
                            const isAfterArrival = savedArrivalDate && incomingDate && incomingDate > savedArrivalDate;

                            return (
                              <div className={`p-3 rounded-xl border text-xs font-bold flex items-center gap-2 ${isBeforeInitial || isBeforeArrival
                                ? "bg-amber-50 border-amber-200 text-amber-900"
                                : isSameArrival || isAfterArrival
                                ...
                                <span>
                                  {isBeforeInitial
                                    ? `टीप: निवडलेली तारीख (${incomingDate}) ही आरंभीची शिल्लक नोंदवलेल्या दिनांकाच्या (${initialStockDate}) आधीची असल्यामुळे साठा ० (0) दर्शवत आहे.`
                                    : isBeforeArrival
                                      ? `टीप: निवडलेली तारीख (${incomingDate}) ही आवक दिनांकाच्या (${savedArrivalDate}) आधीची असल्यामुळे आवक साठा जोडलेला नाही. एकूण उपलब्ध साठा = मागील शिल्लक.`
                                      : isSameArrival
                                        ? `साहित्य आवक तारीख (${incomingDate}): मागील शिल्लक + प्राप्त आवक साहित्याची बेरीज करून एकूण उपलब्ध साठा दर्शवला आहे.`
                                        : isAfterArrival
                                          ? `साहित्य आवक नंतरची तारीख (${incomingDate}): मागील शिल्लक + आवक साहित्याची बेरीज करून एकूण उपलब्ध साठा दर्शवला आहे.`
                                          : `साहित्य आवक तारीख निवडा. निवडलेल्या तारखेनुसार उपलब्ध साठ्याची बेरीज आपोआप होईल.`}
                                </span>
                              </div>
                            );
                          })()}
                          ...
                          <div className="w-full overflow-x-auto">
                            <table className="w-full text-left text-xs border-collapse min-w-[750px]">
                              ...
                              <tbody className="divide-y divide-slate-200">
                                {REPORT_ITEMS.slice(0, 18).map((item, idx) => {
                                  const isBeforeInitial = !!(initialStockDate && incomingDate && incomingDate < initialStockDate);
                                  const openVal = isBeforeInitial ? 0 : roundStock(getOpeningStock(incomingMonth, incomingYear, incomingClass, item.key));
                                  const currentRecKey = `${incomingYear}_${incomingMonth}_${incomingClass}`;
                                  const savedArrivalDate = incomingDates[currentRecKey];
                                  ...
                                  if (!isBeforeInitial) {
                                    incRecords.forEach((r) => {
                                      ...
                                      if (isMatch) {
                                        if (!incomingDate || r.date <= incomingDate) {
                                          const parts = r.date.split("-");
                                          if (parts.length === 3) {
                                            const recYear = parts[0];
                                            const monthIndex = parseInt(parts[1], 10) - 1;
                                  ...
                                  const isBeforeArrivalDate = isBeforeInitial || !!(savedArrivalDate && incomingDate && incomingDate < savedArrivalDate);
                                  const typedIncVal = isBeforeArrivalDate ? 0 : (parseFloat(incomingQuantities[item.key] || "0") || 0);
                                    ...
                                    <tr key={item.key} className={`hover:bg-emerald-50/40 transition-colors ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                                      ...
                                      <td className="p-2 border-r border-slate-200 text-center bg-blue-50/30">
                                        <input
                                          ...
                                          min="0"
                                          placeholder="0"
                                          value={totalIncVal > 0 ? totalIncVal.toString() : (incomingQuantities[item.key] || "")}
                                          onChange={(e) =>
                                            setIncomingQuantities({
                                              ...incomingQuantities,
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6">
                          ...
                          {(() => {
                            ...
                            return (
                              <div className={`p-3 rounded-xl border text-xs font-bold flex items-center gap-2 ${isBeforeInitial || isBeforeLok
                                ? "bg-amber-50 border-amber-200 text-amber-900"
                                    ...
                                    <tr key={item.key} className={`hover:bg-emerald-50/40 transition-colors ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                                      ...
                                      <td className="p-2 border-r border-slate-200 text-center bg-amber-50/30">
                                        <input
                                          ...
                                          min="0"
                                          placeholder="0"
                                          value={totalLokVal > 0 ? totalLokVal.toString() : (openingStockLoksahabhag[item.key] || "")}
                                          onChange={(e) => handleOpeningLoksahabhagChange(item.key, e.target.value)}
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6">
                          ...
                          {(() => {
                            ...
                            return (
                              <div className={`p-3 rounded-xl border text-xs font-bold flex items-center gap-2 ${isBeforeInitial || isBeforeDmg
                                ? "bg-amber-50 border-amber-200 text-amber-900"
                                    ...
                                    <tr key={item.key} className={`hover:bg-emerald-50/40 transition-colors ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                                      ...
                                      <td className="p-2 border-r border-slate-200 text-center bg-rose-50/30">
                                        <input
                                          ...
                                          min="0"
                                          placeholder="0"
                                          value={totalDmgVal > 0 ? totalDmgVal.toString() : (openingStockSpent[item.key] || "")}
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-slate-200/90 shadow-xs space-y-5">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <label
                              ...
                              className={`p-4 rounded-xl border-2 transition-all cursor-pointer flex items-start gap-3 ${formulaSource === "admin"
                            ...
                            <label
                              ...
                              className={`p-4 rounded-xl border-2 transition-all cursor-pointer flex items-start gap-3 ${formulaSource === "custom"
                                ? "border-emerald-600 bg-emerald-50/40 shadow-xs"
                                : "border-slate-200 bg-white hover:border-slate-300"
                                }`}
                            >
                                      ...
                                      <td className="p-2 text-right">
                                        <div className="flex justify-end">
                                          <input
                                            ...
                                            className={`w-28 h-9 text-center border rounded-lg font-bold text-xs shadow-2xs transition-all ${formulaSource === "admin"
                      ...
                      <div className="w-full space-y-4 font-sans">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                          ...
                          <div className="lg:col-span-5 space-y-4">
                              ...
                              <div className="flex items-center gap-2">
                                <div className="relative flex-1">
                                  <input
                                    type="text"
                                    readOnly
                                    value={formatDateDDMMYYYY(registerDate)}
                                    onClick={() => {
                                      const el = document.getElementById("mdm-register-date-picker");
                                    ...
                                    className="w-full h-10 px-3 pr-10 bg-white border border-slate-300 rounded-lg text-sm font-extrabold text-slate-900 focus:outline-none focus:border-indigo-600 cursor-pointer shadow-2xs"
                                    placeholder="DD/MM/YYYY"
                                  />
                                  <input
                                    id="mdm-register-date-picker"
                                    type="date"
                                    value={registerDate}
                                    onChange={(e) => setRegisterDate(e.target.value)}
                                    className="absolute inset-0 opacity-0 cursor-pointer w-full h-full z-10"
                                  />
                                  <Calendar className="absolute right-3 top-1/2 -translate-y-1/2 size-5 text-indigo-600 pointer-events-none z-20 stroke-[2.5]" />
                                </div>
                                <button
                                  onClick={() => {
                                    const el = document.getElementById("mdm-register-date-picker");
                                  ...
                                  className="h-10 px-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm rounded-lg shadow-sm transition-colors cursor-pointer"
                                >
                                  Go
                                </button>
                              </div>
                            ...
                            <div className="bg-emerald-50/70 rounded-xl border border-emerald-200 p-4 space-y-1">
                              <div className="flex items-center justify-between text-emerald-900 font-extrabold text-sm">
                                <span>{formatDateDDMMYYYY(registerDate)} · {getTranslatedDay(registerDay)}</span>
                              </div>
                                ...
                                <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-4">
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-start">
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        className={`w-full h-10 px-3 border rounded-xl text-center text-sm font-bold text-slate-800 placeholder:text-slate-400 placeholder:font-bold focus:ring-2 outline-none ${isRegisterDisabled
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        className={`w-full h-10 px-3 border rounded-xl text-center text-sm font-bold placeholder:text-slate-400 placeholder:font-bold focus:ring-2 outline-none ${isRegisterDisabled
                                          ? "bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed"
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        className={`w-full h-10 px-3 border rounded-xl text-center text-sm font-bold focus:ring-2 outline-none ${isRegisterDisabled
                                          ? "bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed"
                                      ...
                                      <div className="bg-emerald-50/60 border border-emerald-200 rounded-xl p-3.5 space-y-0">
                                        ...
                                        <div className="space-y-1 pt-1">
                                          ...
                                          <input
                                            ...
                                            className={`w-full h-9 px-3 border rounded-lg text-sm font-bold text-right text-slate-800 focus:outline-none focus:border-emerald-500 ${isRegisterDisabled ? "bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed" : "bg-white border-slate-300"
                                              }`}
                                          />
                                        </div>
                                      </div>
                                      ...
                                      {purakAhar && (
                                        <div className="space-y-1 pt-1">
                                          ...
                                          <textarea
                                            ...
                                            className={`w-full p-2.5 border rounded-lg text-xs font-medium text-slate-800 focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 outline-none transition-all shadow-xs ${isRegisterDisabled ? "bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed" : "bg-white border-indigo-300"
                                              }`}
                                          />
                                        </div>
                                      )}
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <textarea
                                      ...
                                      className={`w-full p-2.5 border rounded-lg text-xs font-medium focus:outline-none ${isRegisterDisabled ? "bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed" : "bg-white border-slate-300 focus:border-slate-400"
                                        }`}
                                    />
                                  </div>
                                  ...
                                  <button
                                    ...
                                    className={`w-full py-2.5 rounded-lg shadow-sm transition-colors flex items-center justify-center gap-2 font-bold text-sm ${isRegisterDisabled
                                      ? "bg-slate-300 text-slate-500 cursor-not-allowed"
                          ...
                          <div className="lg:col-span-7 bg-white rounded-xl border border-slate-200 shadow-sm p-4 space-y-3">
                            ...
                            {/* Top Horizontal Scrollbar Bar & Indicator */}
                            <div className="space-y-1">
                              <div className="flex items-center justify-between px-1">
                                <span className="text-[11px] font-extrabold text-indigo-700 flex items-center gap-1">
                                  <span>↔️</span> पुढील माहिती पाहण्यासाठी वरचा स्क्रोल बार (Top Scrollbar):
                                </span>
                              </div>
                              <div
                                ref={topScrollRef}
                                onScroll={handleTopScroll}
                                className="w-full overflow-x-auto overflow-y-hidden border border-indigo-200 rounded-t-lg bg-indigo-50/80 p-0.5 cursor-pointer shadow-inner"
                              >
                            ...
                            <div
                              ...
                              className="w-full overflow-x-auto max-h-[620px] overflow-y-auto border border-slate-200 border-t-0 rounded-b-lg shadow-2xs"
                            >
                                  ...
                                  {(() => {
                                    ...
                                    for (let d = 1; d <= daysInMonth; d++) {
                                      ...
                                      rows.push(
                                        <tr
                                          key={fullDateKey}
                                          className={`transition-colors ${isCurrentSelectedDate
                            ...
                            <div className="space-y-0">
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  setStockYear(newYear);
                                  if (newYear !== "Select Year" && stockMonth !== "Select Month") {
                                    setStockAsOnDate(getLastDayOfMonth(newYear, stockMonth));
                                  }
                                }}
                            ...
                            <div className="space-y-0">
                              ...
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  setStockMonth(newMonth);
                                  if (stockYear !== "Select Year" && newMonth !== "Select Month") {
                                    setStockAsOnDate(getLastDayOfMonth(stockYear, newMonth));
                                  }
                                }}
                                className="w-full h-10 px-3 bg-white border border-[#ccc] rounded shadow-none text-sm font-normal text-slate-800 outline-none focus:border-slate-400"
                              >
                        ...
                        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3">
                          <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3 sm:gap-4">
                            ...
                            <div className="space-y-1">
                              ...
                              <select
                                value={stockDemandClass}
                                onChange={(e) => {
                                  const newCls = e.target.value as "1 To 5" | "6 To 8";
                                  setStockDemandClass(newCls);
                                  const newPat = getDynamicPatSankhya(profile, newCls);
                                  if (newPat > 0) setStockDemandPatSankhya(newPat.toString());
                                }}
                                className="h-10 px-3 bg-white border border-slate-300 rounded-lg text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-indigo-500 outline-none min-w-[170px]"
                              >
                                <option value="1 To 5">इयत्ता १ ली ते ५ वी (प्राथमिक)</option>
                                <option value="6 To 8">इयत्ता ६ वी ते ८ वी (उच्च प्राथमिक)</option>
                              </select>
                        ...
                        <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-300 shadow-md space-y-4 print:p-0 print:border-none print:shadow-none font-sans text-slate-900">
                          <div id="stock-demand-report-print" className="bg-white p-3 space-y-3 border border-slate-300 rounded-xl">
                            {(() => {
                              ...
                              const regData = getRegisterDataForMonth(monthEng, actualYear, stockDemandClass);
                        ...
                        <div className="bg-white p-2 sm:p-4 md:p-6 rounded-2xl border border-slate-300 shadow-sm space-y-3 print:p-0 print:border-none print:shadow-none w-full max-w-full overflow-x-hidden">
                          <div id="monthly-mdm-report-print" className="space-y-0 bg-white p-2 text-slate-900 font-sans">
                            ...
                            {monthlyMdmReportType === "daily_tandul_register" && (() => {
                              ...
                              const prevRiceStock = getOpeningStock(engMonthNames[monthNum], year.toString(), monthlyReportClass, "Rice");
                              ...
                              for (let day = 1; day <= daysInMonth; day++) {
                                ...
                                const daily = getDailyDataForMonthDate(dateISO, monthlyReportClass);
                                  ...
                                  <div className="space-y-2 flex-1 flex flex-col">
                                    ...
                                    <div className="w-full flex-1 flex flex-col overflow-x-auto">
                                      <table className="w-full min-w-[750px] sm:min-w-[850px] h-full min-h-[850px] border-collapse border border-slate-700 text-center text-[14px] md:text-[15px] font-black table-fixed">
                                        <thead>
                                          <tr className="bg-white text-slate-900 font-black border-b border-slate-700 text-[15px]">
                                          ...
                                          <tr className="bg-white text-slate-800 font-black border-b border-slate-700 text-[14px]">
                                        ...
                                        <tbody>
                                          {monthRows.map((r) => {
                                            ...
                                            return (
                                              <tr
                                                key={r.sr}
                                                className={`border-b border-slate-700 text-[15px] font-black h-11 ${isSundayOrHoliday
                                                  ? "bg-[#fce8e6]"
                                                  : "bg-white"
                                                  }`}
                                              >
                                    ...
                                    <div className="flex-1 flex flex-col justify-between h-full space-y-2">
                                      {isFirstPage && (
                                        <div className="text-center mb-4 space-y-1">
                                          ...
                                          <p className="text-xs font-semibold text-slate-600">पोषण आहार दैनंदिन नोंदी — {monthYearStr} · {monthlyReportClass === "6 To 8" ? "उच्च प्राथमिक ( इयत्ता ६ ते ८ )" : "प्राथमिक ( इयत्ता १ ते ५ )"}</p>
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <div className="w-full overflow-x-auto">
                                      <table className="w-full min-w-[1100px] border-collapse border border-slate-700 text-center text-xs font-medium" style={{ tableLayout: 'fixed' }}>
                                        ...
                                        <thead>
                                          <tr className="bg-white text-slate-900 font-bold border-b border-slate-700 text-xs">
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center" colSpan={4}>शाळा व लाभार्थी माहिती</th>
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center" colSpan={4}>तांदूळ स्थिती (KG)</th>
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center" colSpan={3}>तांदूळ वापर व शिल्लक (KG)</th>
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center" colSpan={3}>शा.पो.आ. शिजवण्यासाठी चालू महिन्याचा खर्च</th>
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center" colSpan={2}>दिवस माहिती</th>
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center" colSpan={1}>मागणी</th>
                                          </tr>
                                          <tr className="bg-white text-slate-900 font-bold border-b border-slate-700 text-xs">
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center leading-tight">अ.न.<br /><span className="font-normal">1</span></th>
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center leading-tight">शाळेचे नाव<br /><span className="font-normal">2</span></th>
                                            <th className="border-r border-slate-700 px-0.5 py-1 font-bold align-middle bg-white text-center leading-tight">पट<br /><span className="font-normal">3</span></th>
                                            <th className="border-r border-slate-700 px-0.5 py-1 font-bold align-middle bg-white text-center leading-tight">लाभार्थी<br /><span className="font-normal">4</span></th>
                                            <th className="border-r border-slate-700 px-0.5 py-1 font-bold align-middle bg-white text-center leading-tight">मागील शिल्लक तांदूळ<br /><span className="font-normal">6</span></th>
                                            <th className="border-r border-slate-700 px-0.5 py-1 font-bold align-middle bg-white text-center leading-tight">प्राप्त तांदूळ<br /><span className="font-normal">7</span></th>
                                            <th className="border-r border-slate-700 px-0.5 py-1 font-bold align-middle bg-white text-center leading-tight">उसना घेतलेला तांदूळ<br /><span className="font-normal">8</span></th>
                                            <th className="border-r border-slate-700 px-0.5 py-1 font-bold align-middle bg-white text-center leading-tight">एकूण तांदूळ<br /><span className="font-normal">(6+7+8)</span></th>
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center leading-tight">या महिन्यात शिजवून दिलेला तांदूळ (KG)<br /><span className="font-normal">9</span></th>
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center leading-tight">उसना परत केलेला तांदूळ (KG)<br /><span className="font-normal">10</span></th>
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center leading-tight">शिल्लक तांदूळ (KG)<br /><span className="font-normal">11</span></th>
                                            <th className="border-r border-slate-700 px-0.5 py-1 font-bold align-middle bg-white text-center leading-tight">केंद्र हिस्सा 1.56<br /><span className="font-normal">12</span></th>
                                            <th className="border-r border-slate-700 px-0.5 py-1 font-bold align-middle bg-white text-center leading-tight">राज्य हिस्सा 1.03<br /><span className="font-normal">13</span></th>
                                            <th className="border-r border-slate-700 px-0.5 py-1 font-bold align-middle bg-white text-center leading-tight">2.59<br /><span className="font-normal">14</span></th>
                                            <th className="border-r border-slate-700 px-0.5 py-1 font-bold align-middle bg-white text-center leading-tight">कामाचे दिवस<br /><span className="font-normal">15</span></th>
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center leading-tight">प्रत्यक्ष शा.पो.आ. शिजवून दिल्याचे दिवस<br /><span className="font-normal">16</span></th>
                                            <th className="border-r border-slate-700 px-1 py-1 font-bold align-middle bg-white text-center leading-tight">पुढील मागणी (KG)<br /><span className="font-normal">17</span></th>
                                          </tr>
                                        </thead>
                                        <tbody>
                            ...
                            {monthlyMdmReportType === "masik_tandul_bill" && (() => {
                              ...
                              const marDays = ["रविवार", "सोमवार", "मंगळवार", "बुधवार", "गुरुवार", "शुक्रवार", "शनिवार"];
                              const schoolName = profile?.schoolName || "";
                              const shasDar = monthlyReportClass === "6 To 8" ? (Number(upperRate) || 8.17) : (Number(primaryRate) || 5.45);
                              const rates = getFoodGrantRates(monthlyReportClass, shasDar);
                              const kendraRate = rates.centerRate;
                              const rajyaRate = rates.stateRate;
                              ...
                              const dailyData = Array.from({ length: daysInMonth }, (_, i) => {
                                ...
                                const isSunday = date.getDay() === 0;
                                const dateISO = `${year}-${String(monthNum).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
                                const dailyEntry = getDailyDataForMonthDate(dateISO, monthlyReportClass);
                                const pat = dailyEntry?.enrolled || getDynamicPatSankhya(profile, monthlyReportClass);
                          ...
                          <div className="bg-[#f8fafc] p-3 md:p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-3.5 sm:gap-4">
                            ...
                            <div className="flex flex-wrap items-center bg-slate-200/80 p-1 rounded-xl gap-1 w-full sm:w-auto justify-center">
                              <button
                                ...
                                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all cursor-pointer ${monthlySubTab === "1-5"
                              ...
                              <button
                                ...
                                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all cursor-pointer ${monthlySubTab === "6-8"
                                  ? "bg-white text-blue-700 shadow-sm font-extrabold"
                                  : "text-slate-600 hover:text-slate-900"
                                  }`}
                              >
                              ...
                              <button
                                ...
                                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all cursor-pointer ${monthlySubTab === "1-8"
                                  ? "bg-white text-blue-700 shadow-sm font-extrabold"
                                  : "text-slate-600 hover:text-slate-900"
                                  }`}
                              >
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                ...
                                className={`px-3 py-1.5 rounded-xl text-xs font-extrabold flex items-center gap-1.5 transition-all cursor-pointer border ${showCertEditor
                              ...
                              <button
                                ...
                                className={`px-3 py-1.5 font-extrabold text-xs rounded-xl shadow-sm transition-all cursor-pointer active:scale-95 border ${selectedReportCategory === "masik_goshwara"
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              {["2023-24", "2024-25", "2025-26", "2026-27", "2027-28", "2028-29"].map((y) => (
                                <button
                                  ...
                                  className={`py-3 px-4 rounded-xl border-2 font-bold text-xs transition-all ${annualReportYear === y
