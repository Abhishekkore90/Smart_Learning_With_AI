...
function SemesterResult9th10th() {
  ...
  const fetchMarksData = (student, subject) => {
    if (
      student &&
      student.result &&
      student.result[academicYear] &&
      student.result[academicYear][selectedExamName] &&
      student.result[academicYear][selectedExamName][subject]
    ) {
      return student.result[academicYear][selectedExamName][subject];
    } else if (
      student &&
      student.icseResult &&
      student.icseResult[academicYear] &&
      student.icseResult[academicYear][selectedExamName] &&
      student.icseResult[academicYear][selectedExamName][subject]
    ) {
            ...
            if (selectedExamName === "Semester First & Second") {
              ...
              semesters.forEach((exam) => {
                ...
                if (
                  fbStudent &&
                  fbStudent.result &&
                  fbStudent.result[academicYear] &&
                  fbStudent.result[academicYear][exam] &&
                  fbStudent.result[academicYear][exam][subject]
                ) {
                  examMarks = fbStudent.result[academicYear][exam][subject];
                } else if (
                  fbStudent &&
                  fbStudent.icseResult &&
                  fbStudent.icseResult[academicYear] &&
                  fbStudent.icseResult[academicYear][exam] &&
                  fbStudent.icseResult[academicYear][exam][subject]
                ) {
                  examMarks = fbStudent.icseResult[academicYear][exam][subject];
                }

                if (
                  examMarks.obtainMarks !== undefined &&
                  examMarks.obtainMarks !== ""
                ) {
                ...
                if (
                  examMarks.totalMarks !== undefined &&
              ...
              if (
                ...
                fbStudent.result &&
                fbStudent.result[academicYear] &&
                fbStudent.result[academicYear][selectedExamName] &&
                fbStudent.result[academicYear][selectedExamName][subject]
              ) {
                marks =
                  fbStudent.result[academicYear][selectedExamName][subject];
              } else if (
                ...
                fbStudent.icseResult &&
                fbStudent.icseResult[academicYear] &&
                fbStudent.icseResult[academicYear][selectedExamName] &&
                fbStudent.icseResult[academicYear][selectedExamName][subject]
              ) {
                marks =
                  fbStudent.icseResult[academicYear][selectedExamName][subject];
              } else {
  ...
  const nonGradedSubjects = subjects.filter(
    (s) =>
      !gradedSubjects.some((g) => s.toLowerCase().includes(g.toLowerCase())),
  );
  ...
  const getTransformedSubjects = () => {
    ...
    nonGradedSubjects.forEach((s) => {
      ...
      if (
        lower.includes("भाग -1") ||
        lower.includes("भाग -2") ||
        lower.includes("भाग-1") ||
        lower.includes("भाग-2") ||
      ...
      if (
        ...
        lower.includes("बीजगणित") ||
        lower.includes("भूमिती") ||
        lower.includes("गणित")
      ) {
        if (!hasMath) {
          newSubs.push("गणित");
        ...
        lower.includes("भूगोल") ||
        lower.includes("समाजशास्त्र")
      ) {
        if (!hasSocialScience) {
  ...
  const getCombinedSubjectMark = (studentSrNo, displaySubject) => {
    ...
    const isScience =
      displaySubject === "Science" || displaySubject === "विज्ञान";
    const isSocialScience =
      displaySubject === "Social Science" || displaySubject === "समाजशास्त्र";
    ...
    if (isMath) {
      ...
      return allAb
        ? { value: 0, text: "AB" }
        : { value: totalValue, text: totalValue };
    }
          ...
          >
            ...
            <tbody>
              {selectedStudents.map((student, index) => {
                const { totalObtained, totalMax, allAbsent } = calculateTotal(
                  student.srNo,
                );
                ...
                return (
                  <tr key={index}>
                    ...
                    <td style={{ textAlign: "center" }}>
                      {allAbsent ? "AB" : totalObtained}
                    </td>
                    <td style={{ textAlign: "center" }}>
                      {allAbsent ? "AB" : `${percentage}%`}
                    </td>
                  </tr>
                );
              })}
              {selectedStudents.length === 0 && (
                <tr>
                  <td
                    colSpan={displaySubjects.length * 2 + 5}
