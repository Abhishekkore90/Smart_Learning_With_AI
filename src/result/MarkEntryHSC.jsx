...
=======
      const studentMarks = await (student.srNo,
      academicYear,
      selectedExamName,
      subject);
      marksData[student.srNo] = studentMarks;
...
=======
              ...
              const obtainMarks =
                writtenMarks +
                oralMarks +
                (type === "graceMarks" ? value : graceMarks);
...
=======
      ...
      const subjectData = {
        ...
        graceMarks: marksData[srNo]?.[subject]?.graceMarks || 0,
        obtainMarks:
          (marksData[srNo]?.[subject]?.writtenMarks || 0) +
          (marksData[srNo]?.[subject]?.oralMarks || 0) +
          (marksData[srNo]?.[subject]?.graceMarks || 0),
        subtype: "Mark",
      };
  ...
  const saveMarks = async () => {
    ...
    try {
      await saveMark1();
      setAlertMessage(
        language === "English" ? "Marks saved successfully" : "गुण जतन केले",
      );
    } catch (error) {
      console.error("Error saving marks and additional fields:", error);
    }
  ...
  const saveGrades = async () => {
    ...
    try {
      ...
      for (const student of selectedStudents) {
        ...
        // Prepare Firebase update
        const firebasePath = `/schoolRegister/${udiseNumber}/studentData/${student.srNo}/result/${academicYear}/${selectedExamName}/${subject}`;
        updates[firebasePath] = subjectData;
      }
...
=======
    return (
      divisionSubjects[classValue] &&
      divisionSubjects[classValue][division] &&
      divisionSubjects[classValue][division].length > 0
    );
  };
  // Add this useEffect to filter students when the subject changes
  useEffect(() => {
      ...
      <div>
        <div className="main-content-of-page">
          <h3 style={{ color: "rgb(3, 54, 94)", textAlign: "center" }}>
            {" "}
            {language === "English" ? "HSC Marks Entry" : "गुण नोंदणी"}
          </h3>
          <div
            style={{
              display: "flex",
          ...
          >
            <table
              className="table table-striped table-bordered me-3"
              style={{ flex: "2" }}
            >
              <tbody>
                <tr>
                  ...
                  <td>
                    <div style={{ display: "flex", alignItems: "center" }}>
                      ...
                      >
                        ...
                        {divisionSubjects[classValue] &&
                        divisionSubjects[classValue][division]
                          ? divisionSubjects[classValue][division].map(
                              (sub) => (
                                <option key={sub} value={sub}>
                                  {sub}
                                </option>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (!subject) {
                      setAlertMessage(
                        language === "English"
                          ? "Please select a subject before entering marks!"
                          : "कृपया गुण प्रविष्ट करण्यापूर्वी विषय निवडा!",
                      );
                      setMinMarks(""); // Reset the field
                      return;
                    }
                    if (value < 0) {
                      setAlertMessage(
                        language === "English"
                          ...
                          : "गुण 0 पेक्षा कमी असू शकत नाहीत!",
                      );
                      setMinMarks(""); // Reset the field
                      return;
                    }
            ...
            <table className="table table-striped table-bordered">
              <thead>
                <tr>
                  ...
                  <th colSpan={"2"} className="text-center">
                    {" "}
                    {language === "English"
                      ? "Student Result Remark for all subject"
                      : "शेरा"}
                  </th>
                </tr>
              </thead>
              <tbody>
                {selectedStudents
                  ...
                  .map((student) => {
                    const studentMarks =
                      marksData[student.srNo]?.[subject] || {}; // Fetch marks for the current student and subject
                    return (
                      <tr key={student.srNo}>
                        <td>{student.rollNo}</td>
                        <td>
                        ...
                        <td>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              const totalMarks =
                                value +
                                (studentMarks.oralMarks || 0) +
                                (studentMarks.graceMarks || 0);

                              if (totalMarks > outOfMarks) {
                                alert(
                                  "Total marks cannot exceed Out Of marks!",
                                );
                              } else {
                                handleMarksChange(
                                  student.srNo,
                        ...
                        {/* Obtain Marks */}
                        <td>
                          {studentMarks.writtenMarks +
                            studentMarks.oralMarks +
                            studentMarks.graceMarks || 0}
                        </td>
