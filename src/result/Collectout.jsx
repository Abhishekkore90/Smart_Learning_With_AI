      ...
      const fetchMarks = async () => {
        ...
        selectedStudents.forEach((student, index) => {
          ...
          subjectList.forEach((sub, subjectIndex) => {
            newMarksData[student.srNo][sub] =
              marksDataArray[index * subjectList.length + subjectIndex] || {};
          });
        });
        ...
        try {
          const term =
            selectedExamName === "First Semester" ? "first" : "second";
          ...
          console.warn(
            "Firestore marks fetch failed for Collectout, using IndexedDB data:",
            fsError,
          );
        }
      };
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (
            data &&
            !initialYear &&
            !localStorage.getItem("cce_academic_year")
          ) {
  ...
  const fetchStudentData = async () => {
    try {
      ...
      try {
        ...
        console.warn(
          "Firebase student data fetch failed, checking IndexedDB:",
          firebaseError,
        );
      }
      ...
      activeStudents.forEach((student) => {
        if (student && student.currentClass) {
          ...
          classesAndDivisions[student.currentClass][div].push(
            student.id || student.srNo,
          );
        }
      });
  ...
  const handlePrint = () => {
    ...
    if (tableElement) {
      ...
      if (Object.keys(subjects).length === 0) {
        setAlertMessage(
          language === "English"
            ? "No subject sequence configured."
            : "विषय यादी उपलब्ध नाही.",
        );
        return;
      }
      ...
      const printHeader = (hasLogo = true) => {
        const logoHtml = hasLogo
          ? `<img src="${finalSchoolLogo}" alt="School Logo">`
          : "";
        printWindow.document.write(`
        ...
        const processedTableContent = tableContent
          ...
          .replace(
            /<th class="rotate-print-header">([^<]+)<\/th>/g,
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-white tracking-tight">
              {language === "English"
                ? "Compilation Form"
                : "श्रेणीनिहाय-निकाल-संकलन-प्रपत्र"}
            </h2>
                  ...
                  <tr>
                    ...
                    {Object.keys(subjects).map((subj, index) => (
                      <th
                        key={index}
                        colSpan="4"
                        className="p-3 border border-emerald-950/40 font-bold text-emerald-400"
                      >
                        {subj}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-emerald-950/30">
                  {[...selectedStudents]
                    ...
                    .map((student) => (
                      <tr
                        key={student.srNo}
                        className="hover:bg-[#121c16]/40 transition-colors"
                      >
                        ...
                        {Object.keys(subjects).map((subj, idx) => {
                          const akarTotal =
                            marksData[student.srNo]?.[subj]?.Akarik?.Total ??
                            "-";
                          const sankTotal =
                            marksData[student.srNo]?.[subj]?.Sanklik?.Total ??
                            "-";
                          const totalObt =
                            (marksData[student.srNo]?.[subj]?.Akarik?.Total ??
                              0) +
                          ...
                          return (
                            <React.Fragment key={idx}>
                              <td className="p-2 border border-emerald-950/40 text-[#a2d8b4]/90">
                                {akarTotal}
                              </td>
                              <td className="p-2 border border-emerald-950/40 text-[#a2d8b4]/90">
                                {sankTotal}
                              </td>
                              <td className="p-2 border border-emerald-950/40 font-semibold text-white">
                                {hasMarks ? totalObt : "-"}
                              </td>
                              <td
                                className="p-2 border border-emerald-950/40 font-bold"
                                style={{ color: getGradeColor(totalObt) }}
                              >
