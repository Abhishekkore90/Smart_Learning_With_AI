...
=======
  const [allMarks, setAllMarks] = useState<
    Record<string, Record<string, SubjectMarks>>
  >({});
>>>>>>> dbeff7e14a4166b051f7c9a6dda16ad16f4ca557
...
=======
        ...
        if (settingsSnap.exists() && settingsSnap.data().subjects)
          setSubjects(settingsSnap.data().subjects);
        const marksSnap = await getDoc(
          doc(
            db,
            "cce_marks_v2",
            `${selectedClass}_${academicYear}_${selectedExamKey}`,
          ),
        );
        setAllMarks(marksSnap.exists() ? marksSnap.data().records || {} : {});
      } catch {
...
=======
          const oldSnap = await getDoc(
            doc(db, "cce_weightage", `${selectedClass}_${academicYear}`),
          );
>>>>>>> dbeff7e14a4166b051f7c9a6dda16ad16f4ca557
...
=======
      upakramKriti:
        parseInt((record.upakramKriti ?? record.upakram) as any) || 0,
      prakalpa: parseInt(record.prakalpa as any) || 0,
      chaachaniLekhi:
        parseInt((record.chaachaniLekhi ?? record.chaachani) as any) || 0,
      swadhyayVargakarya:
        parseInt((record.swadhyayVargakarya ?? record.swadhyay) as any) || 0,
>>>>>>> dbeff7e14a4166b051f7c9a6dda16ad16f4ca557
...
=======
    ...
    setAllMarks((prev) => ({
      ...
      [studentId]: {
        ...(prev[studentId] || {}),
        [subjectName]: {
          ...getSubjectMarks(studentId, subjectName),
          [field]: value,
        },
>>>>>>> dbeff7e14a4166b051f7c9a6dda16ad16f4ca557
      },
...
=======
    const sw =
      assignedItem.subjects[subjectName] ||
      assignedItem.subjects[getSubjectKey(subjectName)];
>>>>>>> dbeff7e14a4166b051f7c9a6dda16ad16f4ca557
    if (!sw) {
      return defaultCols;
    }
...
=======
      {
        key: "tondiKaam",
        label: "तोंडीकाम",
        max: parseInt(sw.tondiKaam) || 0,
        type: "akarik",
      },
      {
        key: "pratyakshikPrayog",
        label: "प्रात्याक्षिक / प्रयोग",
        max: parseInt(sw.pratyakshikPrayog) || 0,
        type: "akarik",
      },
      {
        key: "upakramKriti",
        label: "उपक्रम / कृती",
        max: parseInt(sw.upakramKriti) || 0,
        type: "akarik",
      },
      {
        key: "prakalpa",
        label: "प्रकल्प",
        max: parseInt(sw.prakalpa) || 0,
        type: "akarik",
      },
      {
        key: "chaachaniLekhi",
        label: "चाचणी (लेखी)",
        max: parseInt(sw.chaachaniLekhi) || 0,
        type: "akarik",
      },
      {
        key: "swadhyayVargakarya",
        label: "स्वाध्याय / वर्गकार्य",
        max: parseInt(sw.swadhyayVargakarya) || 0,
        type: "akarik",
      },
      {
        key: "itar",
        label: "इतर",
        max: parseInt(sw.itar) || 0,
        type: "akarik",
      },
      {
        key: "sankalitTondi",
        label: "तोंडी",
        max: parseInt(sw.sankalitTondi) || 0,
        type: "sankalit",
      },
      {
        key: "sankalitPratyakshik",
        label: "प्रात्यक्षिक",
        max: parseInt(sw.sankalitPratyakshik) || 0,
        type: "sankalit",
      },
      {
        key: "sankalitLekhi",
        label: "लेखी",
        max: parseInt(sw.sankalitLekhi) || 0,
        type: "sankalit",
      },
    ];
  ...
  const isSubjectFilledForStudent = (
    studentId: string,
    rollNoStr: string,
    subjectName: string,
  ): boolean => {
...
=======
    students.forEach((student) => {
      if (
        isSubjectFilledForStudent(student.id, student.rollNo || "", subjectName)
      ) {
>>>>>>> dbeff7e14a4166b051f7c9a6dda16ad16f4ca557
        filled++;
      }
...
=======
    const akarikTotal = akarikCols.reduce(
      (sum, c) => sum + (sm[c.key] || 0),
      0,
    );
    const sankalitTotal = sankalitCols.reduce(
      (sum, c) => sum + (sm[c.key] || 0),
      0,
    );
    ...
    return (
      <div
        className="rounded-[2.5rem] border shadow-2xl min-h-[600px] flex flex-col relative select-none overflow-hidden"
      ...
      >
        ...
        >
          <div
            className="w-8 h-8 rounded-full font-bold text-sm flex items-center justify-center border flex-shrink-0"
            style={{
...
=======
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm" style={{ color: T.textLo }}>
                {subjectIndex + 1}/{subjects.length}
              </span>
              <button
                onClick={() =>
                  setSubjectIndex(
                    Math.min(subjects.length - 1, subjectIndex + 1),
                  )
                }
...
=======
              ...
              <div className="grid grid-cols-2 gap-3">
                {akarikCols.map((col) => (
                  <div key={col.key}>
                    <p className="text-xs mb-1 text-slate-650 font-bold">
                      {col.label} ({col.max})
                    </p>
                    <MarksInput
                      value={sm[col.key] || 0}
                      max={col.max}
                      onChange={(v) =>
                        setSubjectMark(student.id, subject, col.key, v)
                      }
                    />
>>>>>>> dbeff7e14a4166b051f7c9a6dda16ad16f4ca557
                  </div>
...
=======
        <div
          className="absolute bottom-0 left-0 right-0 px-5 pb-5 pt-3 flex gap-3"
          style={{
...
=======
      ...
      >
        ...
        <div
          className="flex items-center justify-between px-5 py-2.5 flex-shrink-0 bg-slate-50/50"
          style={{ borderBottom: `1px solid ${T.border}` }}
        >
...
=======
            ...
            students.map((student, idx) => {
              const sm = getSubjectMarks(student.id, subject);
              const activeCols = getActiveColsForStudent(
                student.rollNo || "",
                subject,
              );
              const total = activeCols.reduce(
                (sum, col) => sum + (sm[col.key] || 0),
                0,
              );
              const totalMax = activeCols.reduce(
                (sum, col) => sum + col.max,
                0,
              );
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <div
                        className="w-8 h-8 rounded-full font-bold text-xs flex items-center justify-center border flex-shrink-0"
                        style={{
                          background: T.accentDim,
                          color: T.accent,
                          borderColor: T.border,
                  ...
                  <div className="overflow-x-auto pb-1">
                    ...
                    >
                      {activeCols.map((col) => (
                        ...
                        >
                          <p
                            className="text-[10px] mb-1 truncate text-slate-500 font-bold"
                            title={`${col.label} (${col.max})`}
                          >
                            {col.label} ({col.max})
                          </p>
                          <div
                            className="flex items-center rounded-xl overflow-hidden h-11"
                            style={{
                              background: T.input,
                          ...
                          >
                            <input
                              ...
                              min="0"
                              value={
                                (sm[col.key] as number) === 0 ? "" : sm[col.key]
                              }
                              onChange={(e) =>
                                setSubjectMark(
                                  student.id,
                              ...
                              style={{ color: T.textHi }}
                            />
                            <span className="pr-1.5 text-[10px] text-slate-400 font-bold">
                              /{col.max}
                            </span>
                          </div>
                        </div>
        ...
        <div className="flex p-1 rounded-xl" style={{ background: T.card }}>
          {(["sem1", "sem2"] as Semester[]).map((sem) => (
            <button
              ...
              style={{
                background:
                  activeSemester === sem ? T.accentDim : "transparent",
                color: activeSemester === sem ? T.accent : T.textLo,
              }}
            >
>>>>>>> dbeff7e14a4166b051f7c9a6dda16ad16f4ca557
...
=======
      <div
        className="px-4 py-3 overflow-x-auto flex-shrink-0"
        style={{ borderBottom: `1px solid ${T.border}` }}
      >
        <div className="flex items-center gap-2">
          {currentExams.map((e) => (
            <button
              key={e.key}
              onClick={() => setSelectedExamKey(e.key)}
>>>>>>> dbeff7e14a4166b051f7c9a6dda16ad16f4ca557
...
=======
            ...
            />
            <span className="text-xs font-bold" style={{ color: T.textLo }}>
              लोड होत आहे...
            </span>
>>>>>>> dbeff7e14a4166b051f7c9a6dda16ad16f4ca557
          </div>
...
=======
              ...
              students.map((student, idx) => {
                ...
                return (
                  <div
                    key={student.id}
                    className="flex items-center justify-between px-2 py-3.5 rounded-xl transition-colors cursor-pointer"
                    style={{ background: "transparent" }}
                    onClick={() => {
                  ...
                  >
                    ...
                    {filled === 0 ? (
                      ...
                      <div
                        className="w-9 h-9 rounded-full border-2 flex items-center justify-center flex-shrink-0 font-bold text-[11px]"
                        style={{
                          borderColor: T.accent,
                          color: T.accent,
                          background: "transparent",
...
=======
                <div
                  key={sub}
                  className="flex items-center justify-between px-2 py-4 rounded-xl transition-colors cursor-pointer"
                  style={{ background: "transparent" }}
                  onClick={() => setEditingSubject(sub)}
                  onMouseEnter={(e) =>
                    (e.currentTarget.style.background = T.card)
