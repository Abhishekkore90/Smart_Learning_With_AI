...
export default function ExactMathsQuestionBank() {
  ...
  // 1. Purge Old Data & Read Maths Sheet Directly
  const handleFileUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        let targetSheetName = wb.SheetNames[0] || ""; // default
        let targetData: any[][] = [];

        // Find Sheet matching "Maths" or "Math" or "गणित"
        for (const sName of wb.SheetNames) {
          ...
          const contentStr = JSON.stringify(sheetData).toLowerCase();
          const sNameLower = sName.toLowerCase().trim();

          if (
            sNameLower.includes("math") ||
            sNameLower.includes("गणित") ||
            contentStr.includes("विषय - maths") ||
            contentStr.includes("विषय:- maths") ||
            contentStr.includes("maths") ||
            contentStr.includes("गणित")
          ) {
        ...
        if (targetData.length === 0 && wb.SheetNames.length > 0) {
          const ws = wb.Sheets[wb.SheetNames[0]];
          targetData = XLSX.utils.sheet_to_json(ws, {
            header: 1,
            defval: "",
            blankrows: true,
          });
        ...
        setDetectedSheet(targetSheetName);
        setRawRows(targetData);
        toast.success(
          `✅ "${file.name}" (Maths Sheet: ${targetSheetName}) यशस्वीरित्या फेच झाली!`
        );
      } catch (err: any) {
        console.error("Maths QB Excel parse error:", err);
  ...
  // 2. Parse & Group Rows (Parent SrNo & Topic Span)
  const parsedData = useMemo(() => {
    ...
    let formNo = "08";
    let std = "इयत्ता १ ली (1st)";
    let subject = "Maths (गणित)";
  ...
  const handleExportPDF = async () => {
    try {
      ...
      )} | प्रपत्र क्रमांक - ${escHtml(metadata.formNo)}  प्रश्नपेढी</th></tr>`;
      ...
      toast.success("🎉 Maths प्रश्नपेढी PDF यशस्वीरित्या डाउनलोड झाली!");
    } catch (err: any) {
      setIsExportingPdf(false);
      console.error("Maths QB PDF Error:", err);
  ...
  return (
    <div className="w-full flex flex-col gap-4 p-4 bg-slate-100 min-h-screen text-slate-900 font-sans rounded-2xl">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-indigo-950">
              इयत्ता १ ली - Maths (गणित) प्रश्नपेढी डॅशबोर्ड
            </h2>
            <p className="text-xs text-slate-500">
              {fileName
                ? `फाईल: ${fileName} | फेच केलेली शीट: ${detectedSheet}`
                : "कृपया Class 1 Maths प्रश्नपेढी Excel फाईल (.xlsx) अपलोड करा"}
            </p>
          </div>
        </div>
      ...
      {/* RENDER TARGET - EXACT MERGED BOX GRID */}
      <div
        id="exact-maths-qb-root"
            ...
            <tbody>
              {filteredGroups.length > 0 ? (
                ...
                <tr>
                  ...
                  >
                    📂 Class 1 Maths साठी फाईल निवडून अपलोड करा.
                  </td>
                </tr>
              )}
            </tbody>
