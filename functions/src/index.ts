...
export const generateAIResponse = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  ...
  try {
    ...
    if (model === "chatgpt") {
      ...
      if (!response.ok) {
        if (response.status === 401) throw new functions.https.HttpsError("unauthenticated", "Invalid OpenAI API Key.");
        if (response.status === 429) throw new functions.https.HttpsError("resource-exhausted", "Rate limited or out of OpenAI credits.");
        throw new functions.https.HttpsError("unknown", `OpenAI API returned status ${response.status}`);
      }
      const result = await response.json();
      return { text: result.choices?.[0]?.message?.content || "No response." };
    }
    ...
    // -------------------------------------------------------------
      ...
      if (!response.ok) {
        if (response.status === 401) throw new functions.https.HttpsError("unauthenticated", "Invalid Claude API Key.");
    ...
    // -------------------------------------------------------------
      ...
      if (!response.ok) {
        if (response.status === 401 || response.status === 403) throw new functions.https.HttpsError("unauthenticated", "Invalid Gemini API Key.");
        if (response.status === 429) throw new functions.https.HttpsError("resource-exhausted", "Rate limited on Gemini API.");
